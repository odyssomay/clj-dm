...
(defclass apply-rules-window (dialog)
  ((logfile :initarg :logfile :accessor logfile :initform (make-instance 'logfile))
   ...
   (rule-interaction :initarg :rule-interaction :accessor rule-interaction :initform nil)
   (interaction-factor :initarg :interaction-factor :accessor interaction-factor :initform "2")))
...
(defun make-apply-rules-window (&key (parent *dm-main-window*) 
  ...
  (make-window name
    ...
    (list 
     ...
     (make-instance 'check-box 
       :name :check-box-rule-interaction 
       :title "Rule interact:"
       :value nil
       :box (make-box 2 194 100 216) 
       :tabstop nil
       :set-value-fn #'(lambda (widget new old)
                         (setf (rule-interaction (parent widget)) new)
                         (values t ;; Accept the new value
                                 nil))
       :font (make-font-ex :swiss :arial 12)
       :background-color t
     ...
     (make-instance 'editable-text 
       ...
       :value "2" 
       :box (make-box 50 218 100 240)
       :tabstop t 
       :tooltip "Interaction compression factor (1 no compression, >1 compression)"
       :font (make-font-ex :modern :courier\ new 12)
       :set-value-fn #'(lambda (widget new old)
                         (setf (interaction-factor (parent widget)) new)
                         (values t ;; Accept the new value
                                 nil)) )
     (make-instance 'editable-text 
...
;new version with rule interaction but no log
;120305/af
(defmethod apply-rules ((item apply-rules-window))
  (if (rule-interaction item)
      (rule-interaction-apply-rules-sync (rule-list item) (read-from-string (interaction-factor item)))
    (rule-apply-list-sync (rule-list item)(sync-type item))
    )
  (redraw-display-windows)
  )
